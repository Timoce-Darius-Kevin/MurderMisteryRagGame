...
class ErrorHandler:
    ...
    def _setup_logger(self, log_file: str) -> logging.Logger:
        ...
        Multiple :class:`ErrorHandler` instances share the same named logger.
        To avoid duplicate log lines, handlers are only attached once.
        """
        logger = logging.getLogger("MurderMysteryGame")
        logger.setLevel(logging.DEBUG)

        # If handlers are already configured, reuse them.
        if logger.handlers:
            return logger
